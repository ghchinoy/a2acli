...
```bash
a2acli invoke "Please calculate the fibonacci of 10" --service-url http://localhost:9001
```

You can also pass supplemental instructions via a file:
```bash
a2acli invoke "Analyze this data" --instruction-file ./instructions.txt
```

To target a specific skill:
```bash
a2acli invoke "Use the calculator" --skill "calc_skill"
```

To save artifacts generated by the agent:
```bash
a2acli invoke "Generate a report" --out-dir ./output/
```

#### Resume a Task
If you disconnected from a long-running task, you can resume listening to its event stream:
```bash
a2acli resume <task_id> --service-url http://localhost:9001
```

#### Check Task Status
Retrieve the current state of a task without streaming:
```bash
a2acli status <task_id> --service-url http://localhost:9001
```
